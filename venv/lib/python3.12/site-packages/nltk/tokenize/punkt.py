...
class PunktTokenizer(PunktSentenceTokenizer):
    ...
    def load_lang(self, lang="english"):
        ...
        try:
            # Attempt to find the language resource in the punkt directory
            lang_dir = find(f"tokenizers/punkt/{lang}.pickle")
            print(f"lang_dir successfully loaded: {lang_dir}")  # Debugging output
        except LookupError:
            # Raise an error if the resource is not found
            raise LookupError(f"Language resource for {lang} not found.")

        # Load parameters for the tokenizer
...
def load_punkt_params(lang_dir):
    ...
    # Load the Punkt parameters directly from the pickle file
